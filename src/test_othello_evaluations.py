...
class TestCustomEvaluation(unittest.TestCase):
    ...
    def test_evaluate_custom(self):
        """
        Testa a função de avaliação customizada.
        """

        # Caso de teste 1: Tabuleiro inicial
        state1 = Board.from_string("""........
........
........
...WB...
...
........""")
        game_state1 = GameState(state1, 'B')
        expected_score1_B = 0  # Valores posicionais e outros fatores são neutros no início
        score1_B = evaluate_custom(game_state1, 'B')
        self.assertAlmostEqual(score1_B, expected_score1_B, delta=1, msg="Erro no estado inicial (Jogador B).")

        # Caso de teste 2: Estado intermediário com relevância para bordas e mobilidade
        state2 = Board.from_string("""........
........
.....B..
...WBW..
...
........""")
        game_state2 = GameState(state2, 'B')
        expected_score2_B = 5  # Valores aproximados considerando posição, mobilidade e bordas
        score2_B = evaluate_custom(game_state2, 'B')
        self.assertAlmostEqual(score2_B, expected_score2_B, delta=1, msg="Erro no estado intermediário (Jogador B).")

        # Caso de teste 3: Estado complexo com cantos ocupados
        state3 = Board.from_string('BBBBBBB.\n'
                                   'BBWWWWBB\n'
                                   'BWBWWBBB\n'
                                   'BWWWBBBB\n'
                                   ...
                                   'BBBWBBBB\n')
        game_state3 = GameState(state3, 'B')
        expected_score3_B = 50  # Relevância para cantos e estabilidade
        score3_B = evaluate_custom(game_state3, 'B')
        self.assertAlmostEqual(score3_B, expected_score3_B, delta=5, msg="Erro no estado complexo (Jogador B).")

        # Caso de teste 4: Estado terminal - Vitória de 'B'
        state4 = Board.from_string("""BBBBBBBB
BBBBBBBB
BBBBBBBB
BBBBBBBB
BBBBBBBB
...
BBBBBBBB""")
        game_state4 = GameState(state4, 'B')
        expected_score4_B = float('inf')  # Vitória para B
        score4_B = evaluate_custom(game_state4, 'B')
        self.assertEqual(score4_B, expected_score4_B, "Erro no estado terminal - Vitória para B.")

        # Caso de teste 5: Estado terminal - Vitória de 'W'
        state5 = Board.from_string("""WWWWWWWW
WWWWWWWW
WWWWWWWW
WWWWWWWW
WWWWWWWW
...
WWWWWWWW
WWWWWWWW""")
        game_state5 = GameState(state5, 'B')
        expected_score5_B = float('-inf')  # Vitória para W
        score5_B = evaluate_custom(game_state5, 'B')
        self.assertEqual(score5_B, expected_score5_B, "Erro no estado terminal - Vitória para W.")

        # Caso de teste 6: Estado terminal - Empate
        state6 = Board.from_string("""WWWWWWWW
WWWWWWWW
WWWWWWWW
WWWWWWWW
BBBBBBBB
...
BBBBBBBB
BBBBBBBB""")
        game_state6 = GameState(state6, 'B')
        expected_score6_B = 0  # Empate
        score6_B = evaluate_custom(game_state6, 'B')
        self.assertEqual(score6_B, expected_score6_B, "Erro no estado terminal - Empate.")
